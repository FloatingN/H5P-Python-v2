...
H5P.BranchingPython = function (params, contentId) {
  const self = this;
  ...
  }, params.branchingPython); // Account for the wrapper!
  ...
  self.params = params;
  self.scoring = new H5P.BranchingPython.Scoring(params);
   ...
   * @param  {boolean} isCurrentScreen When Branching Python is first initialized
   * @return {H5P.BranchingPython.GenericScreen} Generic Screen object
   */
  const createStartScreen = function ({ startScreenTitle, startScreenSubtitle, startScreenImage, startScreenAltText }, isCurrentScreen) {
    const startScreen = new H5P.BranchingPython.GenericScreen(self, {
      isStartScreen: true,
  ...
  const createEndScreen = function (endScreenData) {
    const endScreen = new H5P.BranchingPython.GenericScreen(self, {
      isStartScreen: false,
  ...
  /**
   * Get library data by id from branching python parameters
   *
  ...
  /**
   * Handle the start of the branching python
   */
  ...
  self.on('navigated', function (e) {
    ...
    if (self.currentId > -1 && H5P.BranchingPython.LibraryScreen.isBranching(self.getLibrary(self.currentId)) && self.$container.find('.h5p-back-button[isBQ="true"]').length) {
    ...
    if (!self.libraryScreen) {
      self.libraryScreen = new H5P.BranchingPython.LibraryScreen(
        self,
      ...
      if (!this.params.preventXAPI) {
        ...
        const isBranching = H5P.BranchingPython.LibraryScreen.isBranching(self.getLibrary(self.currentId));
    ...
    if (H5P.BranchingPython.LibraryScreen.isBranching(self.getLibrary(self.currentId))) {
    ...
    if (self.currentEndScreen && self.currentEndScreen.isShowing) {
      if (nextLibrary) {
        if (!H5P.BranchingPython.LibraryScreen.isBranching(nextLibrary)) {
  ...
  self.on('restarted', function () {
    ...
    self.libraryScreen = new H5P.BranchingPython.LibraryScreen(self, params.startScreen.startScreenTitle, self.getLibrary(0));
  ...
  /**
   * Get accumulative score for all attempted pythons
   *
   * @returns {number} Current score for Branching Python
   */
   ...
   * Get max score
   *
   * @returns {number} Max score for branching python
   */
  ...
  /**
   * Attach Branching Python to the H5P container
   *
  ...
  self.attach = function ($container) {
    ...
    $container.addClass('h5p-branching-python').html('');
...
H5P.BranchingPython.prototype = Object.create(H5P.EventDispatcher.prototype);
H5P.BranchingPython.prototype.constructor = H5P.BranchingPython;
