  ...
  static getDerivedStateFromProps(props) {
    return ({ // Update selected option
      selectedMainOption: props.nextContentId >= 0
        ? 'old-content'
        : (BranchingOptions.hasFeedback(props) ? 'end-python' : 'new-content')
    });
  }
  ...
  componentDidUpdate() {
    if (this.props.feedbackGroup && this.props.feedbackGroup !== undefined) {
      ...
      if (this.state.selectedMainOption !== 'new-content') {
        ...
        this.props.feedbackGroup.$title.html(this.state.selectedMainOption === 'end-python' ? t('feedbackOnEndScreen') : this.props.feedbackGroup.field.label);
        this.props.feedbackGroup.children[0].$item.find('.h5peditor-label').html(this.state.selectedMainOption === 'end-python' ? t('feedbackTitle') : this.props.feedbackGroup.children[0].field.label);
        this.props.feedbackGroup.children[1].$item.find('.h5peditor-label').html(this.state.selectedMainOption === 'end-python' ? t('feedbackText') : this.props.feedbackGroup.children[1].field.label);
        this.props.feedbackGroup.children[2].$item.children('.h5peditor-label-wrapper').children('.h5peditor-label').html(this.state.selectedMainOption === 'end-python' ? t('feedbackImage') : this.props.feedbackGroup.children[2].field.label);

        const isEndPython = !(this.props.nextContentId > -1);

        // Cases that need the scoring option field
        const isDynamicScore = (this.props.scoringOption === 'dynamic-score') && !isEndPython;
        const isStaticScore = (this.props.scoringOption === 'static-end-score') && isEndPython;
