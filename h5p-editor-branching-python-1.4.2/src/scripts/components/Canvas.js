...
export default class Canvas extends React.Component {
   ...
   */
  componentDidMount() {
    // Trigger the initial default end pythons count
    this.props.onContentChanged(null, this.countDefaultEndPythons());
  ...
  handleContentHighlight = (newContentType, currentContentType) => {
    let newContentTypeTitle, python = 'NEW_CONTENT_ON_EXISTING_CONTENT';
    // Placing existing content on another existing content
    if (this.state.placing > -1) {
      python = 'EXISTING_CONTENT_ON_EXISTING_CONTENT';
    }
    ...
    if (content.params.type.params.branchingQuestion) {
      python = 'NEW_CONTENT_ON_EXISTING_BQ';
      if (this.props.inserting && this.props.inserting.pasted) {
        newContentTypeTitle = this.props.inserting.pasted.generic.metadata.title;
        python = 'PASTED_CONTENT_ON_EXISTING_BQ';
      }
      // Placing existing content on existing branching question
      if (this.state.placing > -1) {
        python = 'EXISTING_CONTENT_ON_EXISTING_BQ';
      }
    }
  ...
  handleDropzoneHighlight = (existingContentId) => {
    if (this.state.insertingId) {
      ...
      this.setState({
        tips: {
          python: 'NEW_CONTENT_ON_DROPZONE',
    ...
    if (this.state.placing > -1) {
      this.setState({
        tips: {
          python: 'EXISTING_CONTENT_ON_DROPZONE',
    ...
    if (this.props.inserting && this.props.inserting.pasted) {
      ...
      this.setState({
        tips: {
          python: 'PASTED_CONTENT_ON_DROPZONE',
  ...
  getParent = (id, contentNodes) => {
    return this.tree.processed
      ...
      .filter(candidate => candidate > -1) // except end pythons
  ...
  handleContentCopy = (id) => {
    const clipboardItem = new H5P.ClipboardItem(this.state.content[id].params, 'type', 'H5PEditor.BranchingPython');
  ...
  getNewContentParams = () => {
    if (this.props.inserting && this.props.inserting.pasted) {
      ...
      if (this.props.inserting.pasted.from === 'H5PEditor.BranchingPython' &&
      ...
      if (id === -1) {
        ...
        if (id === 0) {
          if (!isBranching(newState.content[0])) {
            newState.content[0].params.nextContentId = -1; // Use default end python as default end python
          }
  ...
  static getBranchingChildren(content) {
    ...
    content.params.type.params.branchingQuestion.alternatives.forEach(alternative => {
      children.push(alternative.nextContentId); // Other question or end python
    });
  ...
  /**
   * Go through content state and count how many default end pythons
   * that are present.
   *
   * @return {number}
   */
  countDefaultEndPythons = () => {
    let numMissingEndPythons = 0;
    this.state.content.forEach(content => {
      if (isBranching(content)) {
        const alternatives = content.params.type.params.branchingQuestion.alternatives || [];
        alternatives.forEach(alternative => {
          if (this.hasEndScreen(alternative) && !this.hasCustomEndScreen(alternative)) {
            numMissingEndPythons++;
          }
        });
      }
