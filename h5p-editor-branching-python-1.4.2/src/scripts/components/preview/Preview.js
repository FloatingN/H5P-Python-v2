...
export default class Preview extends React.Component {
  ...
  initializeInstance() {
    // Do not initialize empty branching pythons
    if (this.props.params.branchingPython.content.length <= 0) {
      return;
    }
    ...
    const branchingPython = Object.keys(H5PEditor.libraryLoaded)
      .filter((library) => {
        return library.split(' ')[0] === 'H5P.BranchingPython';
      })[0];

    // Flag passed into BS to prevent XAPI statements being fired whilst in Preview.
    this.props.params.branchingPython.preventXAPI = true;

    this.preview = H5P.newRunnable(
      {
        library: branchingPython,
