  ...
  render() {
    ...
    return (
      <div className={ 'toolbar' + (this.props.isTourActive ? ' tour-fade' : '') }>
        ...
        <div
          className="missing-end-pythons"
          title={`${this.props.numDefaultEndPythons} ${t('alternativeWithoutCustomEnd')}`}
          role="button"
          tabIndex="0"
          onClick={ this.handleShowPopup }
        >{ this.props.numDefaultEndPythons }</div>
        <div className={ infoPopupClass }>
          { this.props.numDefaultEndPythons } {(t('alternativesMissing'))}
          { !!this.props.numDefaultEndPythons &&
            <span> <span className="highlight-end-pythons-button" role="button" tabIndex="0" onClick={ this.handleHighlight }>{t('clickHereToHighlight')}</span></span>
          }
...
Toolbar.propTypes = {
  numDefaultEndPythons: PropTypes.number,
