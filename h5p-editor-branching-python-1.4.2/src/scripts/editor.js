...
export default class Editor extends React.Component {
  ...
  componentDidMount() {
    ...
    if (H5PEditor.semiFullscreen !== undefined) {
      getUserStorage('h5p-editor-branching-python-tour-v1-seen', (seen) => {
        if (seen !== true) {
  ...
  /**
   * Handle highlighting default end pythons.
   *
   * @param {number} id Id of python to be highlighted.
   * @param {} onlyThisBall
   */
  ...
  handleContentChanged = (content, numDefaultEndPythons) => {
    const newState = {
      numDefaultEndPythons: numDefaultEndPythons
    };
    ...
    if (content !== null && content.length === 1 && this.state.isEditing) {
      if (content[0].type.params.branchingQuestion !== undefined) {
        getUserStorage('h5p-editor-branching-python-information-content-tour-v1-seen', (seen) => {
          if (seen !== true) {
        ...
        getUserStorage('h5p-editor-branching-python-branching-content-tour-v1-seen', (seen) => {
          if (seen !== true) {
  ...
  handleCloseTour = () => {
    if (this.state.tour.state === 'screen-size') {
      ...
      setUserStorage('h5p-editor-branching-python-tour-v1-seen', true);
    }
    else if (this.state.tour.state === 'branching-node') {
      setUserStorage('h5p-editor-branching-python-branching-content-tour-v1-seen', true);
    }
    else if (this.state.tour.state === 'content-node') {
      setUserStorage('h5p-editor-branching-python-information-content-tour-v1-seen', true);
    }
        ...
        <div
          ...
          {this.state.fullscreen &&
            <div
              ...
              title={t('proceedToSaveBranchingPython')}
              role="button"
              tabIndex="0"
              onClick={this.handleToggleFullscreen}
            >{t('proceedToSave')}</div>
        ...
        >
          <Tab
            ...
            <Toolbar
              ...
              numDefaultEndPythons={this.state.numDefaultEndPythons}
              onHighlight={this.handleHighlight}
              scale={this.state.scale}
